...
| Workflows create | PASS (Admin create/delete) | PASS (Viewer `/workflows` has no create control) | PASS (Viewer create denied) | Browser UAT verifies admin create/read-back/delete and Viewer direct URL/API denial for workflow creation. Step mutation/reorder permissions remain outstanding. |
