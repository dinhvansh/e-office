...
| Document type lifecycle and mapping | PASS (read-after-create UI) | Not run | PASS (Admin + matrix negative) | Real UAT verifies mapping persistence, inactive filtering and unused-type deletion with a numbering rule. User document-type creation is denied by the authorization matrix. |
