...
| External organization contact identity | PASS (read-after-create UI) | Not run | PASS (Admin + matrix negative) | Real UAT API/browser lifecycle rejects duplicate code and contact email within the tenant. Authorization matrix confirms User create and Viewer update are denied; cross-tenant negative remains outstanding. |
