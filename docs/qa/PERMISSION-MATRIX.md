...
| Roles lifecycle | PASS (Admin create/delete custom role) | Not run | PASS (Viewer read; Viewer create denied) | Browser UAT confirms a custom role persists, duplicate name is rejected, and unused role deletes. The API matrix confirms current policy permits metadata read but denies creation to Viewer. |
