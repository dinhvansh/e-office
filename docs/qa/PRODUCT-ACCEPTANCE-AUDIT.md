...
| Users / Roles / Permissions | PARTIAL | Admin real-browser lifecycles create a User with Department, Position, and Role assignments and create a custom Role; both persist through the API. User partial update, deactivate/reactivate, password change, duplicate email, and invalid Department/Role inputs are verified; duplicate Role names are rejected and unused roles/users delete. Fresh PostgreSQL API matrix verifies Viewer can read role metadata but cannot create Roles; broader persona/role matrix and role-precedence coverage remain unrun. |
