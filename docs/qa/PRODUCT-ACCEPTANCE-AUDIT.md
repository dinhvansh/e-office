...
| Master Data | PARTIAL | Real UAT browser flows created numbered document `010/2026` from the accessible document-type list and completed Department, Position, and External Organization create/update/delete lifecycles. Duplicate department/position/org codes and external-contact email are rejected; Department/Position disable/re-enable is verified. PostgreSQL confirms `departments.is_active` migration and document-type persistence. The authorization matrix passed Admin create/update for departments, positions and document types, plus Viewer/User/Manager negative master-data checks. Disabled-record downstream filtering and two-tenant UI coverage remain unrun. |
