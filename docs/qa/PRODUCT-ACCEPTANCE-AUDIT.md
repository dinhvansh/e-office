...
| Master Data | PASS | Real UAT browser flows created numbered document `010/2026` from the accessible document-type list and completed Department, Position, External Organization, Document Type, and Tenant/Company lifecycles. Tenant onboarding creates isolated admins; the Settings UI persists company name/domain changes, rejects a duplicate name, and unauthenticated contexts redirect to login. A second tenant cannot see or fetch the first tenant's Department, External Organization, or Document Type in UI/API. External Organization mapping persists on a real draft external signer. Document Type default-workflow mapping persists, inactive types are excluded from the document-upload chooser, and inactive Department/Position records are excluded from User assignment dropdowns. Duplicate department/position/org codes and external-contact email are rejected; Department/Position disable/re-enable is verified. Deleting a Department with a referenced User is safely denied with `DEPARTMENT_HAS_USERS`; once the User is deleted, Department and Position delete cleanly. PostgreSQL confirms `departments.is_active` migration and document-type persistence. A dedicated tenant's inactive status blocks both a new login and an already-issued access token (403); re-enabling restores login. The authorization matrix passed Admin create/update for departments, positions, document types and external organizations, plus Viewer/User/Manager negative master-data checks including Viewer denial for tenant-profile update. |
| Users / Roles / Permissions | PARTIAL | Admin real-browser lifecycles create a User with Department, Position, and Role assignments and create a custom Role; both persist through the API. User partial update, deactivate/reactivate, password change, duplicate email, and invalid Department/Role inputs are verified. A real multi-role user receives the union of permissions from both assigned Roles; duplicate Role names are rejected and unused roles/users delete. Fresh PostgreSQL API matrix verifies Viewer can read role metadata but cannot create Roles; broader persona matrix and removed-workflow-user coverage remain unrun. |
| RBAC / ACL / Tenant Isolation | PARTIAL | Fresh PostgreSQL API matrix verified role denies and cross-tenant document denial; authenticated direct navigation, refresh, and same-session new-tab persistence are verified for the seeded Admin. Viewer direct URLs for Roles and Workflows render read-only UI with mutation controls absent, while their direct create APIs return 403. Real-browser ACL regressions deny both a same-tenant Viewer and a separately provisioned foreign-tenant Admin (HTTP 403) from granting permissions on another user's document. Full tampering matrix remains unverified. |
...
| Sequential signing | PASS | Chromium UAT completes two internal signers in order. Signer 2 remains unavailable until Signer 1 completes, then the worker generates the artifact and the authorized final PDF download succeeds. |
...
| Golden Path 1 | PASS | Chromium UAT verifies configured Document Type/Workflow, My Tasks approval, internal signing, completed artifact, and authorized PDF download. |
| Golden Path 2 | PASS | Chromium UAT verifies two sequential approval steps (Admin then workflow-peer), two sequential internal signers, asynchronous artifact generation, completed state, and authorized PDF download. |
...
13. `BUG-UAT-015` — Sequential approval workflows materialized only the first step. Completion correctly tried to activate the next step's `waiting` records, but those records did not exist, so the next approver never received a task. Submission now materializes all resolved steps and marks later steps `waiting`; the existing atomic transition activates them after the preceding step completes. Chromium regression covers Admin approval → workflow-peer approval → two ordered internal signatures → artifact and PDF download.
