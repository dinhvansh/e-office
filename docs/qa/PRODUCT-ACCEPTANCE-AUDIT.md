...
| Master Data | PARTIAL | Real UAT browser flows created numbered document `010/2026` from the accessible document-type list and completed Department, Position, External Organization, Document Type, and Tenant/Company lifecycles. Tenant onboarding creates isolated admins; the Settings UI persists company name/domain changes, rejects a duplicate name, and unauthenticated contexts redirect to login. A second tenant cannot see or fetch the first tenant's Department in the UI/API. Document Type default-workflow mapping persists, inactive types are excluded from the document-upload chooser, and inactive Department/Position records are excluded from User assignment dropdowns. Duplicate department/position/org codes and external-contact email are rejected; Department/Position disable/re-enable is verified. PostgreSQL confirms `departments.is_active` migration and document-type persistence. The authorization matrix passed Admin create/update for departments, positions, document types and external organizations, plus Viewer/User/Manager negative master-data checks including Viewer denial for tenant-profile update. Full two-tenant UI data-isolation coverage remains unrun beyond the Department case. |
| Users / Roles / Permissions | PARTIAL | Admin real-browser lifecycle creates a custom Role, persists it through the API, rejects a duplicate name, and deletes it. Fresh PostgreSQL API matrix verifies Viewer can read role metadata but cannot create Roles; full user lifecycle and broader persona/role matrix remain unrun. |
...
| Approval | PASS (runtime subset) | Submit → `AWAITING_APPROVAL`; concurrent duplicate approval rejected; transition to signing verified. |
...
| Golden Path 3 | NOT RUN | External OTP signing artifact flow not executed end to end. |
