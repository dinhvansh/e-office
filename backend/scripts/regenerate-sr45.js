...
async function regenerate() {
  try {
    ...
    for (const field of signRequest.fields) {
      console.log(`   Field ${field.id}: ${field.field_type} at page ${field.page_number}`);
    }
    console.log('');
    ...
    const serviceModule = await import(path.resolve(__dirname, '../src/modules/signRequests/pdfGeneration.service.ts'));
    const pdfGenerationService = serviceModule.pdfGenerationService;
