...
export default function NotificationsPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-5 p-4 sm:p-6" aria-live="polite">
    ...
    <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-2xl font-bold">Thông báo</h1><p className="text-sm text-muted-foreground">Lịch sử thông báo của bạn</p></div><Button variant="outline" onClick={() => markAll.mutate()} disabled={markAll.isPending || !notifications.some((item) => !item.is_read)}><CheckCheck className="mr-2 h-4 w-4" />Đánh dấu tất cả đã đọc</Button></header>
