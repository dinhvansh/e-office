  ...
  return (
    <section className="min-w-0 rounded-xl border bg-white p-4 shadow-sm">
      <div className="mb-3 flex flex-col gap-2">
        <h2 className="flex min-w-0 items-center gap-2 font-semibold"><Files className="h-4 w-4 shrink-0" />Tài liệu bổ sung</h2>
        {!readOnly && data?.can_upload ? <><input ref={input} className="sr-only" type="file" onChange={(event) => upload(event.target.files?.[0])} /><Button className="w-full whitespace-nowrap" size="sm" variant="outline" onClick={() => input.current?.click()} disabled={uploading}><FilePlus2 className="mr-2 h-4 w-4 shrink-0" />{uploading ? 'Đang tải...' : 'Thêm tệp'}</Button></> : null}
      </div>
      {readOnly ? <p className="mb-3 text-xs text-slate-500">Tài liệu đã hoàn thành — chỉ xem, không thể thay đổi tệp bổ sung.</p> : null}
      <div className="space-y-2">
        {isLoading ? <p className="text-sm text-slate-500">Đang tải...</p> : data?.attachments.map((attachment) => <div key={attachment.id} className="flex min-w-0 items-center justify-between gap-2 rounded-lg border p-3"><div className="min-w-0"><p className="break-words text-sm font-medium">{attachment.file_name}</p><p className="text-xs text-slate-500">{attachment.status === 'WITHDRAWN' ? `Đã thu hồi${attachment.withdraw_reason ? ` · ${attachment.withdraw_reason}` : ''}` : attachment.attachment_kind}</p></div><div className="flex shrink-0 items-center"><a className="p-2" href={`/api-proxy/documents/${documentId}/attachments/${attachment.id}/download`} title="Tải xuống"><Download className="h-4 w-4" /></a>{!readOnly && attachment.status === 'ACTIVE' ? <Button size="icon" variant="ghost" title="Quản lý tệp"><MoreHorizontal className="h-4 w-4" /></Button> : null}</div></div>)}
      </div>
    </section>
  );
}
